...
export default function App() {
  ...
  const handleLogin = () => {
    if (username && password) {
      // Show an alert with the username and success message
      Alert.alert('Welcome ', `${username} you have successfully logged In`);

      // Clear input fields after clicking login
      setUsername('');
      setPassword('');
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Second part - text ug text input */}
      <View style={styles.textInputContainer}>
        <Text style={styles.heading}>Fisbuk - LoginPage Act 1</Text>
        <Text style={styles.heading}>in React Native</Text>
